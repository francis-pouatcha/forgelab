...
public class ArticleLotEndpoint
{
	...
	@POST
	@Path("/findByArticleName")
	@Produces({ "application/json", "application/xml" })
	@Consumes({ "application/json", "application/xml" })
	public ArticleLotSearchResult findByArticleName(ArticleLotSearchInput searchInput)
	{
		String articleName = "%"+searchInput.getEntity().getArticleName()+"%";
		searchInput.getEntity().setArticleName(articleName);
		List<ArticleLot> resultList = new ArrayList<ArticleLot>() ;
		Long count = null ;
		if(searchInput.isIncludeBreack()){
			count = ejb.countByArticleNameLikeAndStockEqual(searchInput.getEntity());
			resultList = ejb.findByArticleNameLikeAndStockEqual(searchInput);
		}else {
			count = ejb.countArticleNameLikeAndStockUpperThan(searchInput.getEntity());
			resultList = ejb.findByArticleNameLikeAndAndStockUpperThan(searchInput);
		}
		return new ArticleLotSearchResult(count, detach(resultList),
				detach(searchInput));
	}
