...
public abstract class PaymentPaidByController
{
   ...
   protected void disableButton(final PaymentPaidBySelection selection)
   {
      selection.getPaidBy().setDisable(true);
   }

   protected void activateButton(final PaymentPaidBySelection selection)
   {
   }

   protected void bind(final PaymentPaidBySelection selection)
   {
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            selection.getPaidBy().getItems().clear();
            selection.getPaidBy().getItems().addAll(entities);
         }
      });
      ...
      selection.getPaidBy().valueProperty().addListener(new ChangeListener<Customer>()
      {
         @Override
               ...
               Customer newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setPaidBy(new PaymentPaidBy(newValue));
         }
      });

      selection.getPaidBy().armedProperty().addListener(new ChangeListener<Boolean>()
      {
