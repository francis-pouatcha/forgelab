...
@Singleton
public class InventoryItemController extends DomainComponentController
{

   @Inject
   private InventoryItemSearchController searchController;

   @Inject
   private InventoryItemCreateController createController;

   @Inject
   private InventoryItemEditController editController;

   @Inject
   private InventoryItemListController listController;

   @Inject
   private InventoryItemDisplayController displayController;

   @Inject
   private InventoryItemIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<InventoryItem> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<InventoryItem> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<InventoryItem> selectedModelEvent;

   private InventoryItem createModel = new InventoryItem();
   private InventoryItem searchModel = new InventoryItem();
   private InventoryItem selectedModel = new InventoryItem();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<InventoryItem> entities)
   {
   ...
   {
      // if result is empty: display no result.
      // else display list of inventoryItems.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent InventoryItem selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
   ...
   /**
    * Display search form.
    * @param selectedInventoryItem
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent InventoryItem templateEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent InventoryItem selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
