...
@Singleton
public class SupplierInvoiceSearchController implements EntityController
{

   @Inject
   private SupplierInvoiceSearchView searchView;

   @Inject
   private SupplierInvoiceSearchService searchService;
   @Inject
   private ServiceCallFailedEventHandler searchServiceCallFailedEventHandler;

   @Inject
   @EntitySearchDoneEvent
   private Event<SupplierInvoiceSearchResult> searchDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceSearchService s = (SupplierInvoiceSearchService) event.getSource();
            SupplierInvoiceSearchResult searchResult = s.getValue();
            event.consume();
            s.reset();
      ...
      searchView.getResetButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            PropertyReader.copy(new SupplierInvoice(), model);
         }
      });
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent SupplierInvoice templateEntity)
   {
   ...
   private SupplierInvoiceSearchInput buildSearchInputs()
   {
      SupplierInvoiceSearchInput result = new SupplierInvoiceSearchInput();
   ...
   public void handleNewModelEvent(@Observes @SearchModelEvent SupplierInvoice model)
   {
      this.model = model;
      searchView.bind(this.model);
   }

   public void handleEntityListPageIndexChangedEvent(@Observes @EntityListPageIndexChangedEvent SupplierInvoiceSearchResult searchResult)
   {
