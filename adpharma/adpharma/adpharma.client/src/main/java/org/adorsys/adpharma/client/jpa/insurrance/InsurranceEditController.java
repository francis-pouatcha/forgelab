...
@Singleton
public class InsurranceEditController implements EntityController
{

   @Inject
   private InsurranceEditView editView;

   @Inject
   private InsurranceEditService editService;
   @Inject
   ...
   private Event<Insurrance> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<Insurrance> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<Insurrance>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setInsurrance(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            InsurranceEditService s = (InsurranceEditService) event.getSource();
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent Insurrance p)
   {
