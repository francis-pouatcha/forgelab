...
public class CustomerRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Customer> removedEvent;

   @Inject
   private CustomerRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Customer entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Customer entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Customer_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerRemoveService service = (CustomerRemoveService) wse
                  .getSource();
            Customer p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
