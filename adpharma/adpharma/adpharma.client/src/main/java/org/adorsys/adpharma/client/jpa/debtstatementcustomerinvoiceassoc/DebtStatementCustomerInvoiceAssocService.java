...
public class DebtStatementCustomerInvoiceAssocService
{
   ...
   public DebtStatementCustomerInvoiceAssocService()
   {
      ...
      this.target = client.target(serverAddress + "/rest/debtstatementcustomerinvoiceassocs");
   }
   ...
   {
      Entity<DebtStatementCustomerInvoiceAssoc> eCopy = Entity.entity(entity, media);
      return target.request(media).post(eCopy, DebtStatementCustomerInvoiceAssoc.class);
   }
   ...
   {//@PathParam("id")
      // TODO encode id
      return target.path("" + id).request(media).delete(DebtStatementCustomerInvoiceAssoc.class);
   }
   ...
   {
      Entity<DebtStatementCustomerInvoiceAssoc> ent = Entity.entity(entity, media);
      return target.path("" + entity.getId())
            .request(media).put(ent, DebtStatementCustomerInvoiceAssoc.class);
   }
   ...
   {// @PathParam("id") 
      return target.path("" + id).request(media).get(DebtStatementCustomerInvoiceAssoc.class);
   }
   ...
   {
      return target.request(media).get(DebtStatementCustomerInvoiceAssocSearchResult.class);
   }

   // @GET
   // @Produces("application/xml")
   public DebtStatementCustomerInvoiceAssocSearchResult listAll(int start, int max)
   {
      return target.queryParam("start", start).queryParam("max", max)
            .request(media).get(DebtStatementCustomerInvoiceAssocSearchResult.class);
   }
   ...
   {
      Entity<DebtStatementCustomerInvoiceAssocSearchInput> searchInputEntity = Entity.entity(
            searchInput, media);
      return target.path(FIND_BY).request(media).post(
            searchInputEntity, DebtStatementCustomerInvoiceAssocSearchResult.class);
   }
