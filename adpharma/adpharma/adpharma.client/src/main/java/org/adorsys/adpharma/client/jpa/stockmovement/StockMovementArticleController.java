...
public abstract class StockMovementArticleController
{
   ...
   protected void disableButton(final StockMovementArticleSelection selection)
   {
      selection.getArticle().setDisable(true);
   }

   protected void activateButton(final StockMovementArticleSelection selection)
   {
   }

   protected void bind(final StockMovementArticleSelection selection)
   {
      ...
      selection.getArticle().valueProperty().addListener(new ChangeListener<Article>()
      {
               ...
               Article newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setArticle(new StockMovementArticle(newValue));
         }
      });
