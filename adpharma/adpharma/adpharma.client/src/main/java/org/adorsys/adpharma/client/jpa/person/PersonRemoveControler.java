...
public class PersonRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Person> removedEvent;

   @Inject
   private PersonRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Person entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Person entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Person_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PersonRemoveService service = (PersonRemoveService) wse
                  .getSource();
            Person p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
