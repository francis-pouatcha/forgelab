...
@Singleton
public class AgencyController extends DomainComponentController
{

   @Inject
   private AgencySearchController searchController;

   @Inject
   private AgencyCreateController createController;

   @Inject
   private AgencyEditController editController;

   @Inject
   private AgencyListController listController;

   @Inject
   private AgencyDisplayController displayController;

   @Inject
   private AgencyIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Agency> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Agency> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Agency> selectedModelEvent;

   private Agency createModel = new Agency();
   private Agency searchModel = new Agency();
   private Agency selectedModel = new Agency();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Agency> entities)
   {
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Agency selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
    ...
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent Agency templateEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Agency selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
