...
@Singleton
public class StockMovementDisplayController implements EntityController
{
   ...
   private Event<StockMovement> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<StockMovement> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<StockMovement>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<StockMovement>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<StockMovement>>();
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<StockMovement> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<StockMovement> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new StockMovement());
   }
