...
@Singleton
public class VATController extends DomainComponentController
{

   @Inject
   private VATSearchController searchController;

   @Inject
   private VATCreateController createController;

   @Inject
   private VATEditController editController;

   @Inject
   private VATListController listController;

   @Inject
   private VATDisplayController displayController;

   @Inject
   private VATIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<VAT> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<VAT> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<VAT> selectedModelEvent;

   private VAT createModel = new VAT();
   private VAT searchModel = new VAT();
   private VAT selectedModel = new VAT();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<VAT> entities)
   {
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent VAT selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
    ...
    * @param selectedVAT
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent VAT templateEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent VAT selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
