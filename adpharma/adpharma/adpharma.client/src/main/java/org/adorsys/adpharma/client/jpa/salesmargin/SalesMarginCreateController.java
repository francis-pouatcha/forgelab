...
@Singleton
public class SalesMarginCreateController implements EntityController
{

   @Inject
   private SalesMarginCreateView createView;

   @Inject
   private SalesMarginCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<SalesMargin> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<SalesMargin>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SalesMarginCreateService s = (SalesMarginCreateService) event.getSource();
            SalesMargin ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent SalesMargin model)
   {
      this.model = model;
