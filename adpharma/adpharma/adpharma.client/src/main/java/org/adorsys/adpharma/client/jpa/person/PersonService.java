...
public class PersonService
{
   ...
   public PersonService()
   {
      ...
      this.target = client.target(serverAddress + "/rest/persons");
   }
   ...
   {
      Entity<Person> eCopy = Entity.entity(entity, media);
      return target.request(media).post(eCopy, Person.class);
   }
   ...
   {//@PathParam("id")
      // TODO encode id
      return target.path("" + id).request(media).delete(Person.class);
   }
   ...
   {
      Entity<Person> ent = Entity.entity(entity, media);
      return target.path("" + entity.getId())
            .request(media).put(ent, Person.class);
   }
   ...
   {// @PathParam("id") 
      return target.path("" + id).request(media).get(Person.class);
   }

   // @GET
   // @Produces("application/xml")
   public PersonSearchResult listAll()
   {
      return target.request(media).get(PersonSearchResult.class);
   }

   // @GET
   // @Produces("application/xml")
   public PersonSearchResult listAll(int start, int max)
   {
      return target.queryParam("start", start).queryParam("max", max)
            .request(media).get(PersonSearchResult.class);
   }
   ...
   {
      Entity<PersonSearchInput> searchInputEntity = Entity.entity(
            searchInput, media);
      return target.path(FIND_BY).request(media).post(
            searchInputEntity, PersonSearchResult.class);
   }
