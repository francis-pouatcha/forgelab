...
public class VATRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<VAT> removedEvent;

   @Inject
   private VATRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent VAT entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent VAT entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("VAT_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            VATRemoveService service = (VATRemoveService) wse
                  .getSource();
            VAT p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
