...
public abstract class InventoryItemArticleController
{
   ...
   protected void disableButton(final InventoryItemArticleSelection selection)
   {
      selection.getArticle().setDisable(true);
   }

   protected void activateButton(final InventoryItemArticleSelection selection)
   {
   }

   protected void bind(final InventoryItemArticleSelection selection)
   {
      ...
      selection.getArticle().valueProperty().addListener(new ChangeListener<Article>()
      {
               ...
               Article newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setArticle(new InventoryItemArticle(newValue));
         }
      });
