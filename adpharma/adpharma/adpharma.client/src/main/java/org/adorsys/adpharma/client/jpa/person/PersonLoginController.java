...
public abstract class PersonLoginController
{
   ...
   protected Person sourceEntity;

   protected void disableButton(final PersonLoginSelection selection)
   {
      selection.getLogin().setDisable(true);
   }

   protected void activateButton(final PersonLoginSelection selection)
   {
   }

   protected void bind(final PersonLoginSelection selection)
   {
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            selection.getLogin().getItems().clear();
            selection.getLogin().getItems().addAll(entities);
         }
      });
      ...
      selection.getLogin().valueProperty().addListener(new ChangeListener<Login>()
      {
         @Override
               ...
               Login newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setLogin(new PersonLogin(newValue));
         }
      });

      selection.getLogin().armedProperty().addListener(new ChangeListener<Boolean>()
      {
