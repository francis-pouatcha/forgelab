...
@Singleton
public class PersonCreateController implements EntityController
{

   @Inject
   private PersonCreateView createView;

   @Inject
   private PersonCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Person> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Person>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PersonCreateService s = (PersonCreateService) event.getSource();
            Person ent = s.getValue();
            event.consume();
            s.reset();
    ...
    * @param null
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent Person templateEntity)
   {
