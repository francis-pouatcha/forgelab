...
public class PackagingModeRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<PackagingMode> removedEvent;

   @Inject
   private PackagingModeRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent PackagingMode entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent PackagingMode entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("PackagingMode_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PackagingModeRemoveService service = (PackagingModeRemoveService) wse
                  .getSource();
            PackagingMode p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
