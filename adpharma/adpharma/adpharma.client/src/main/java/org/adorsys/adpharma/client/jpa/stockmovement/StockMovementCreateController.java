...
@Singleton
public class StockMovementCreateController implements EntityController
{

   @Inject
   private StockMovementCreateView createView;

   @Inject
   private StockMovementCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<StockMovement> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<StockMovement>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            StockMovementCreateService s = (StockMovementCreateService) event.getSource();
            StockMovement ent = s.getValue();
            event.consume();
            s.reset();
