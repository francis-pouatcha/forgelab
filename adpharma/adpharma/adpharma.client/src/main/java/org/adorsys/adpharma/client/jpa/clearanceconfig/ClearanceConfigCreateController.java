...
@Singleton
public class ClearanceConfigCreateController implements EntityController
{

   @Inject
   private ClearanceConfigCreateView createView;

   @Inject
   private ClearanceConfigCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<ClearanceConfig> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getResetButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         {
            PropertyReader.copy(new ClearanceConfig(), model);
         }
      });
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<ClearanceConfig>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ClearanceConfigCreateService s = (ClearanceConfigCreateService) event.getSource();
            ClearanceConfig ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent ClearanceConfig model)
   {
      this.model = model;
