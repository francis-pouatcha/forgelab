...
@Singleton
public class PersonDisplayController implements EntityController
{
   ...
   private Event<Person> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<Person> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<Person>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<Person>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<Person>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private Person displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<Person> pendingSelectionRequest = pendingSelectionRequestProperty.get();
    ...
    */
   public void handleSelectionEvent(@Observes @EntitySelectionEvent Person selectedEntity)
   {
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<Person> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(Person.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new Person());
   }
