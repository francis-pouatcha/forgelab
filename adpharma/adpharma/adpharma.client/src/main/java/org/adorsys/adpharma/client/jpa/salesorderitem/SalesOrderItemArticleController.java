...
public abstract class SalesOrderItemArticleController
{
   ...
   protected void disableButton(final SalesOrderItemArticleSelection selection)
   {
      selection.getArticle().setDisable(true);
   }

   protected void activateButton(final SalesOrderItemArticleSelection selection)
   {
   }

   protected void bind(final SalesOrderItemArticleSelection selection)
   {
      ...
      selection.getArticle().valueProperty().addListener(new ChangeListener<Article>()
      {
               ...
               Article newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setArticle(new SalesOrderItemArticle(newValue));
         }
      });
