...
public class CompanyRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Company> removedEvent;

   @Inject
   private CompanyRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Company entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Company entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Company_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CompanyRemoveService service = (CompanyRemoveService) wse
                  .getSource();
            Company p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
