...
public class DocumentStoreRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<DocumentStore> removedEvent;

   @Inject
   private DocumentStoreRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent DocumentStore entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent DocumentStore entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("DocumentStore_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DocumentStoreRemoveService service = (DocumentStoreRemoveService) wse
                  .getSource();
            DocumentStore p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
