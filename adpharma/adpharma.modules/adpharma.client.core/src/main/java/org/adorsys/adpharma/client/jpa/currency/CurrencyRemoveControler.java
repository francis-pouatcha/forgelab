...
public class CurrencyRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Currency> removedEvent;

   @Inject
   private CurrencyRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Currency entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Currency entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Currency_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CurrencyRemoveService service = (CurrencyRemoveService) wse
                  .getSource();
            Currency p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
