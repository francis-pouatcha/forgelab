...
@Singleton
public class CurrencyCreateController implements EntityController
{

   @Inject
   private CurrencyCreateView createView;

   @Inject
   private CurrencyCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Currency> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Currency>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CurrencyCreateService s = (CurrencyCreateService) event.getSource();
            Currency ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent Currency model)
   {
      this.model = model;
