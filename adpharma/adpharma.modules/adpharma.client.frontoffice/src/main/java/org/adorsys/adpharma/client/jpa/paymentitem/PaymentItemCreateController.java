...
@Singleton
public class PaymentItemCreateController implements EntityController
{

   @Inject
   private PaymentItemCreateView createView;

   @Inject
   private PaymentItemCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<PaymentItem> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<PaymentItem>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PaymentItemCreateService s = (PaymentItemCreateService) event.getSource();
            PaymentItem ent = s.getValue();
            event.consume();
            s.reset();
