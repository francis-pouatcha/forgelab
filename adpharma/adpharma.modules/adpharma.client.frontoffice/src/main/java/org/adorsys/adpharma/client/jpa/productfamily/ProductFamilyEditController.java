...
@Singleton
public class ProductFamilyEditController implements EntityController
{

   @Inject
   private ProductFamilyEditView editView;

   @Inject
   private ProductFamilyEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ProductFamilyLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ProductFamily> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ProductFamily> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<ProductFamily>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setProductFamily(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProductFamilyEditService s = (ProductFamilyEditService) event.getSource();
            ProductFamily entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ProductFamilyLoadService s = (ProductFamilyLoadService) event.getSource();
            ProductFamily entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent ProductFamily p)
   {
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent ProductFamily model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new ProductFamily(), displayedEntity);
	}
