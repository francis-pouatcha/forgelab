...
public class SalesMarginRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<SalesMargin> removedEvent;

   @Inject
   private SalesMarginRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent SalesMargin entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent SalesMargin entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("SalesMargin_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SalesMarginRemoveService service = (SalesMarginRemoveService) wse
                  .getSource();
            SalesMargin p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
