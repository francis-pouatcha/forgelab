...
@Singleton
public class PackagingModeDisplayController implements EntityController
{

   @Inject
   private PackagingModeDisplayView displayView;
   ...
   private Event<PackagingMode> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<PackagingMode> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<PackagingMode>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<PackagingMode>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<PackagingMode>>();
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<PackagingMode> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<PackagingMode> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(PackagingMode.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new PackagingMode());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent PackagingMode model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new PackagingMode(), displayedEntity);
	}
}