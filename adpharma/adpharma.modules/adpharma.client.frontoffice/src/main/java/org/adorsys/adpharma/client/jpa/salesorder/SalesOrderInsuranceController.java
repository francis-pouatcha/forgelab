...
public abstract class SalesOrderInsuranceController
{
   ...
   private Event<SalesOrderInsuranceSelectionEventData> selectionRequestEvent;

   private SalesOrderInsuranceSelectionEventData pendingSelectionRequest;
   ...
   protected void disableButton(final SalesOrderInsuranceSelection selection, final SalesOrderInsuranceForm form)
   {
      selection.getSelectButton().setDisable(true);
   }

   protected void activateButton(final SalesOrderInsuranceSelection selection, final SalesOrderInsuranceForm form)
   {
   }

   protected void bind(final SalesOrderInsuranceSelection selection, final SalesOrderInsuranceForm form)
   {
      selection.getSelectButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               {
                  pendingSelectionRequest = new SalesOrderInsuranceSelectionEventData(
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            event.consume();
            s.reset();
            sourceEntity.setInsurance(new SalesOrderInsurance(entity));
         }
      });
   ...
   public void handleAssocSelectionResponseEvent(@Observes @AssocSelectionResponseEvent SalesOrderInsuranceSelectionEventData eventData)
   {
            ...
            && eventData.getSourceEntity() == sourceEntity && eventData.getTargetEntity() != null)
      {
         if (sourceEntity != null)
            sourceEntity.setInsurance(new SalesOrderInsurance(eventData.getTargetEntity()));
      }
