...
@Singleton
public class PrescriberEditController implements EntityController
{

   @Inject
   private PrescriberEditView editView;

   @Inject
   private PrescriberEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private PrescriberLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<Prescriber> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<Prescriber> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<Prescriber>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setPrescriber(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PrescriberEditService s = (PrescriberEditService) event.getSource();
            Prescriber entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            PrescriberLoadService s = (PrescriberLoadService) event.getSource();
            Prescriber entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent Prescriber p)
   {
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent Prescriber model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new Prescriber(), displayedEntity);
	}
