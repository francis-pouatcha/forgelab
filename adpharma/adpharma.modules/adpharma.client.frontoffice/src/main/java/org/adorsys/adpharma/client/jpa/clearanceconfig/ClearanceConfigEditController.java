...
@Singleton
public class ClearanceConfigEditController implements EntityController
{

   @Inject
   private ClearanceConfigEditView editView;

   @Inject
   private ClearanceConfigEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ClearanceConfigLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ClearanceConfig> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ClearanceConfig> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<ClearanceConfig>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setClearanceConfig(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ClearanceConfigEditService s = (ClearanceConfigEditService) event.getSource();
            ClearanceConfig entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ClearanceConfigLoadService s = (ClearanceConfigLoadService) event.getSource();
            ClearanceConfig entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent ClearanceConfig p)
   {
