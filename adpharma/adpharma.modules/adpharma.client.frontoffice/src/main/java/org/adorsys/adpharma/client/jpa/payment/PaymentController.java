...
@Singleton
public class PaymentController extends DomainComponentController
{

   @Inject
   private PaymentSearchController searchController;

   @Inject
   private PaymentCreateController createController;

   @Inject
   private PaymentEditController editController;

   @Inject
   private PaymentListController listController;

   @Inject
   private PaymentDisplayController displayController;

   @Inject
   private PaymentIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Payment> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Payment> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Payment> selectedModelEvent;

   private Payment createModel = new Payment();
   private Payment searchModel = new Payment();
   private Payment selectedModel = new Payment();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Payment> entities)
   {
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Payment selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Payment selectedEntity)
   {
