...
@Singleton
public class DisbursementCreateController implements EntityController{
	 @Inject
	   private DisbursementCreateView createView;

	   @Inject
	   private DisbursementProcessService createService;
	   @Inject
	   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

	   @Inject
	   @EntityCreateDoneEvent
	   private Event<Disbursement> createDoneEvent;

	   @Inject
	   @EntitySearchRequestedEvent
	   private Event<Disbursement> searchRequestedEvent;
	   ...
	   public void init()
	   {
	      ...
	      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
	      {
	         ...
	         public void handle(ActionEvent e)
	         {
	            Set<ConstraintViolation<Disbursement>> violations = createView.getView().validate(model);
	    ...
	    * @param createdEntity
	    */
	   public void handleCreatedEvent(@Observes @EntityCreateDoneEvent Disbursement createdEntity)
	   {
	      PropertyReader.copy(new Disbursement(), model);
	   }
