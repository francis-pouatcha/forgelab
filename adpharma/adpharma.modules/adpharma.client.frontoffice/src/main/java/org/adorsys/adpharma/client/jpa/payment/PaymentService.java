...
public class PaymentService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/payments").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public Payment deleteById(Long id) {// @PathParam("id")
										// TODO encode id
	...
	public Payment update(Payment entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, Payment.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public Payment findById(Long id) {// @PathParam("id")
	...
	public PaymentSearchResult findBy(PaymentSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, PaymentSearchResult.class);
	}
	...
	// public Payment customerPayment(Long id , List<CustomerInvoice>
	// customerInvoices)
	// {
	// Entity<List<CustomerInvoice>> ent = Entity.entity(customerInvoices,
	// media);
