...
public class PrescriberCreateController implements EntityController
{
   ...
   @Inject
   private PrescriberCreateView createView;

   @Inject
   private PrescriberCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Prescriber> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Prescriber>> violations = createView.getView().validate(model);
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent Prescriber model)
   {
      this.model = model;
