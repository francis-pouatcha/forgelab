...
@Singleton
public class CustomerInvoiceItemDisplayController implements EntityController
{
   ...
   private Event<CustomerInvoiceItem> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<CustomerInvoiceItem> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<CustomerInvoiceItem>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<CustomerInvoiceItem>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<CustomerInvoiceItem>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private CustomerInvoiceItem displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<CustomerInvoiceItem> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<CustomerInvoiceItem> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new CustomerInvoiceItem());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent CustomerInvoiceItem model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new CustomerInvoiceItem(), displayedEntity);
	}
}