...
@Singleton
public class PaymentItemEditController implements EntityController
{

   @Inject
   private PaymentItemEditView editView;

   @Inject
   private PaymentItemEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private PaymentItemLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<PaymentItem> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<PaymentItem> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<PaymentItem>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setPaymentItem(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PaymentItemEditService s = (PaymentItemEditService) event.getSource();
            PaymentItem entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            PaymentItemLoadService s = (PaymentItemLoadService) event.getSource();
            PaymentItem entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent PaymentItem model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new PaymentItem(), displayedEntity);
	}
