...
@Singleton
public class SalesMarginDisplayController implements EntityController
{

   @Inject
   private SalesMarginDisplayView displayView;
   ...
   private Event<SalesMargin> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<SalesMargin> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<SalesMargin>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<SalesMargin>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<SalesMargin>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private SalesMargin displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<SalesMargin> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<SalesMargin> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(SalesMargin.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new SalesMargin());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent SalesMargin model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new SalesMargin(), displayedEntity);
	}
}