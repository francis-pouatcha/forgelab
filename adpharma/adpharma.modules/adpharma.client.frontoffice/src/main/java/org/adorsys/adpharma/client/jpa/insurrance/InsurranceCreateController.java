...
@Singleton
public class InsurranceCreateController implements EntityController
{

   @Inject
   private InsurranceCreateView createView;

   @Inject
   private InsurranceCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Insurrance> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Insurrance>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            InsurranceCreateService s = (InsurranceCreateService) event.getSource();
            Insurrance ent = s.getValue();
            event.consume();
            s.reset();
