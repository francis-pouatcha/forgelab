...
@Singleton
public class CustomerCategoryDisplayController implements EntityController
{

   @Inject
   private CustomerCategoryDisplayView displayView;
   ...
   private Event<CustomerCategory> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<CustomerCategory> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<CustomerCategory>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<CustomerCategory>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<CustomerCategory>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private CustomerCategory displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<CustomerCategory> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<CustomerCategory> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(CustomerCategory.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new CustomerCategory());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent CustomerCategory model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new CustomerCategory(), displayedEntity);
	}
}