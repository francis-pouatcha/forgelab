...
@Singleton
public class WareHouseEditController implements EntityController
{

   @Inject
   private WareHouseEditView editView;

   @Inject
   private WareHouseEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private WareHouseLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<WareHouse> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<WareHouse> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<WareHouse>> violations = editView.getView().validate(displayedEntity);
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            WareHouseEditService s = (WareHouseEditService) event.getSource();
            WareHouse entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            WareHouseLoadService s = (WareHouseLoadService) event.getSource();
            WareHouse entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent WareHouse p)
   {
