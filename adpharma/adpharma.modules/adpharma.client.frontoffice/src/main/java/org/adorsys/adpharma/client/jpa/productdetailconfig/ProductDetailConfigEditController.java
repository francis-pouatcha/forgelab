...
public class ProductDetailConfigEditController implements EntityController
{
   ...
   @Inject
   private ProductDetailConfigEditView editView;

   @Inject
   private ProductDetailConfigEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ProductDetailConfigLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ProductDetailConfig> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ProductDetailConfig> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProductDetailConfigEditService s = (ProductDetailConfigEditService) event.getSource();
            ProductDetailConfig entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ProductDetailConfigLoadService s = (ProductDetailConfigLoadService) event.getSource();
            ProductDetailConfig entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
