...
public class ClearanceConfigRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<ClearanceConfig> removedEvent;

   @Inject
   private ClearanceConfigRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent ClearanceConfig entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent ClearanceConfig entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("ClearanceConfig_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ClearanceConfigRemoveService service = (ClearanceConfigRemoveService) wse
                  .getSource();
            ClearanceConfig p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
