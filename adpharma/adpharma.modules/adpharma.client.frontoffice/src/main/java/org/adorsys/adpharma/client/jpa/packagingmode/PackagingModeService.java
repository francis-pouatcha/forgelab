...
public class PackagingModeService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/packagingmodes").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public PackagingMode deleteById(Long id) {// @PathParam("id")
												// TODO encode id
		return target().path("" + id).request(media)
				.delete(PackagingMode.class);
	}
	...
	public PackagingMode update(PackagingMode entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, PackagingMode.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public PackagingMode findById(Long id) {// @PathParam("id")
	...
	public PackagingModeSearchResult findBy(PackagingModeSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, PackagingModeSearchResult.class);
	}
	...
	// @Produces("application/xml")
	// @Consumes("application/xml")
	public PackagingModeSearchResult findByLike(
