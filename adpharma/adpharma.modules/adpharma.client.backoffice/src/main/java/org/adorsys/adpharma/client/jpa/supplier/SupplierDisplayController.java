...
@Singleton
public class SupplierDisplayController implements EntityController
{
   ...
   private Event<Supplier> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<Supplier> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<Supplier>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<Supplier>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<Supplier>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private Supplier displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<Supplier> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<Supplier> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(Supplier.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new Supplier());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent Supplier model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new Supplier(), displayedEntity);
	}
}