...
public abstract class DocumentArchiveRecordingUserController
{
   @Inject
   ...
   protected DocumentArchive sourceEntity;

   protected void disableButton(final DocumentArchiveRecordingUserSelection selection)
   {
      selection.getRecordingUser().setDisable(true);
   }

   protected void activateButton(final DocumentArchiveRecordingUserSelection selection)
   {
   }

   protected void bind(final DocumentArchiveRecordingUserSelection selection, final DocumentArchiveRecordingUserForm form)
   {
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            selection.getRecordingUser().getItems().add(new DocumentArchiveRecordingUser());
            for (Login entity : entities)
            {
               selection.getRecordingUser().getItems().add(new DocumentArchiveRecordingUser(entity));
            }
         }
      });
      ...
      selection.getRecordingUser().valueProperty().addListener(new ChangeListener<DocumentArchiveRecordingUser>()
      {
         @Override
         public void changed(ObservableValue<? extends DocumentArchiveRecordingUser> ov, DocumentArchiveRecordingUser oldValue,
               DocumentArchiveRecordingUser newValue)
         {
