...
public class SupplierInvoiceRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<SupplierInvoice> removedEvent;

   @Inject
   private SupplierInvoiceRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent SupplierInvoice entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent SupplierInvoice entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("SupplierInvoice_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceRemoveService service = (SupplierInvoiceRemoveService) wse
                  .getSource();
            SupplierInvoice p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
