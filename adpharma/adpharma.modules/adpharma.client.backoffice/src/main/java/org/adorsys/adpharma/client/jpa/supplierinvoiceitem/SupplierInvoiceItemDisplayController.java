...
@Singleton
public class SupplierInvoiceItemDisplayController implements EntityController
{

   @Inject
   private SupplierInvoiceItemDisplayView displayView;
   ...
   private Event<SupplierInvoiceItem> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<SupplierInvoiceItem> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<SupplierInvoiceItem>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<SupplierInvoiceItem>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<SupplierInvoiceItem>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private SupplierInvoiceItem displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<SupplierInvoiceItem> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<SupplierInvoiceItem> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(SupplierInvoiceItem.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new SupplierInvoiceItem());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent SupplierInvoiceItem model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new SupplierInvoiceItem(), displayedEntity);
	}
}