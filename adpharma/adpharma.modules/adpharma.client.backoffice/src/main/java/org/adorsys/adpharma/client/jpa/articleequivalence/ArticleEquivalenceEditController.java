...
@Singleton
public class ArticleEquivalenceEditController implements EntityController
{

   @Inject
   private ArticleEquivalenceEditView editView;

   @Inject
   private ArticleEquivalenceEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ArticleEquivalenceLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ArticleEquivalence> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ArticleEquivalence> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<ArticleEquivalence>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setArticleEquivalence(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ArticleEquivalenceEditService s = (ArticleEquivalenceEditService) event.getSource();
            ArticleEquivalence entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ArticleEquivalenceLoadService s = (ArticleEquivalenceLoadService) event.getSource();
            ArticleEquivalence entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
