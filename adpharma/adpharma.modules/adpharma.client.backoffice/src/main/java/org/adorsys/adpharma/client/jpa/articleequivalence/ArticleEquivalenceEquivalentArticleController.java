...
public abstract class ArticleEquivalenceEquivalentArticleController
{
   ...
   private Event<ArticleEquivalenceEquivalentArticleSelectionEventData> selectionRequestEvent;

   private ArticleEquivalenceEquivalentArticleSelectionEventData pendingSelectionRequest;
   ...
   protected void disableButton(final ArticleEquivalenceEquivalentArticleSelection selection, final ArticleEquivalenceEquivalentArticleForm form)
   {
      selection.getSelectButton().setDisable(true);
   }

   protected void activateButton(final ArticleEquivalenceEquivalentArticleSelection selection, final ArticleEquivalenceEquivalentArticleForm form)
   {
   }

   protected void bind(final ArticleEquivalenceEquivalentArticleSelection selection, final ArticleEquivalenceEquivalentArticleForm form)
   {
      selection.getSelectButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               {
                  pendingSelectionRequest = new ArticleEquivalenceEquivalentArticleSelectionEventData(
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            event.consume();
            s.reset();
            sourceEntity.setEquivalentArticle(new ArticleEquivalenceEquivalentArticle(entity));
         }
      });
   ...
   public void handleAssocSelectionResponseEvent(@Observes @AssocSelectionResponseEvent ArticleEquivalenceEquivalentArticleSelectionEventData eventData)
   {
            ...
            && eventData.getSourceEntity() == sourceEntity && eventData.getTargetEntity() != null)
      {
         if (sourceEntity != null)
            sourceEntity.setEquivalentArticle(new ArticleEquivalenceEquivalentArticle(eventData.getTargetEntity()));
      }
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(ArticleEquivalence.class.getName()));
   }
