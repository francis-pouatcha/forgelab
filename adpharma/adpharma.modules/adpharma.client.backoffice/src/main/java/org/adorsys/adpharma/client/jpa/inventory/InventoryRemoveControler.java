...
public class InventoryRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Inventory> removedEvent;

   @Inject
   private InventoryRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Inventory entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Inventory entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Inventory_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            InventoryRemoveService service = (InventoryRemoveService) wse
                  .getSource();
            Inventory p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
