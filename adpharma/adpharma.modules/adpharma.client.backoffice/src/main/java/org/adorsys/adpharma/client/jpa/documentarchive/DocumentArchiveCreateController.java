...
@Singleton
public class DocumentArchiveCreateController implements EntityController
{

   @Inject
   private DocumentArchiveCreateView createView;

   @Inject
   private DocumentArchiveCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<DocumentArchive> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<DocumentArchive>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DocumentArchiveCreateService s = (DocumentArchiveCreateService) event.getSource();
            DocumentArchive ent = s.getValue();
            event.consume();
            s.reset();
