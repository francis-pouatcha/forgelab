...
public class DisbursementCashierController {
	 @Inject
	   ...
	   @Inject
	   @Bundle({ CrudKeys.class, Disbursement.class, Login.class })
	   ...
	   protected void disableButton(final DisbursementCashierSelection selection)
	   {
	      selection.getCashier().setDisable(true);
	   }

	   protected void activateButton(final DisbursementCashierSelection selection)
	   {
	   }

	   protected void bind(final DisbursementCashierSelection selection, final DisbursementCashierForm form)
	   {
	      ...
	      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
	      {
	         ...
	         public void handle(WorkerStateEvent event)
	         {
	            ...
	            selection.getCashier().getItems().add(new DisbursementCashier());
	            for (Login entity : entities)
	            {
	               selection.getCashier().getItems().add(new DisbursementCashier(entity));
	            }
	         }
	      });
	      ...
	      selection.getCashier().valueProperty().addListener(new ChangeListener<DisbursementCashier>()
	      {
	         @Override
	         public void changed(ObservableValue<? extends DisbursementCashier> ov, DisbursementCashier oldValue,
	        		 DisbursementCashier newValue)
	         {
