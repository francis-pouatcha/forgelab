...
@Singleton
public class SupplierInvoiceEditController implements EntityController
{

   @Inject
   private SupplierInvoiceEditView editView;

   @Inject
   private SupplierInvoiceEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private SupplierInvoiceLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<SupplierInvoice> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<SupplierInvoice> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<SupplierInvoice>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setSupplierInvoice(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceEditService s = (SupplierInvoiceEditService) event.getSource();
            SupplierInvoice entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            SupplierInvoiceLoadService s = (SupplierInvoiceLoadService) event.getSource();
            SupplierInvoice entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent SupplierInvoice model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new SupplierInvoice(), displayedEntity);
	}
