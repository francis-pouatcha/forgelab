...
public abstract class DeliveryItemArticleController
{
   ...
   protected void disableButton(final DeliveryItemArticleSelection selection)
   {
      selection.getArticle().setDisable(true);
   }

   protected void activateButton(final DeliveryItemArticleSelection selection)
   {
   }

   protected void bind(final DeliveryItemArticleSelection selection)
   {
      ...
      selection.getArticle().valueProperty().addListener(new ChangeListener<Article>()
      {
               ...
               Article newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setArticle(new DeliveryItemArticle(newValue));
         }
      });
