...
@Singleton
public class CurrencyController extends DomainComponentController
{

   @Inject
   private CurrencySearchController searchController;

   @Inject
   private CurrencyCreateController createController;

   @Inject
   private CurrencyEditController editController;

   @Inject
   private CurrencyListController listController;

   @Inject
   private CurrencyDisplayController displayController;

   @Inject
   private CurrencyIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Currency> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Currency> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Currency> selectedModelEvent;

   private Currency createModel = new Currency();
   private Currency searchModel = new Currency();
   private Currency selectedModel = new Currency();

   @Inject
   private CurrencyRegistration registration;
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Currency> entities)
   {
    ...
    */
   public void handleSelectionEvent(@Observes @EntitySelectionEvent Currency selectedEntity)
   {
      if (!registration.canRead())
         return;
      // if result is empty: display no result.
      // else display list of currencys.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Currency selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
    ...
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent Currency templateEntity)
   {
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Currency selectedEntity)
   {
   ...
   public void handleEditCanceledEvent(@Observes @EntityEditCanceledEvent Currency selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
   ...
   public void handleEditDoneEvent(@Observes @EntityEditDoneEvent Currency selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
