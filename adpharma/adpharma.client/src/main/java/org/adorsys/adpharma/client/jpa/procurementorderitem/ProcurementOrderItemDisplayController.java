...
@Singleton
public class ProcurementOrderItemDisplayController implements EntityController
{
   ...
   private Event<ProcurementOrderItem> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<ProcurementOrderItem> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<ProcurementOrderItem>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<ProcurementOrderItem>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<ProcurementOrderItem>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private ProcurementOrderItem displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<ProcurementOrderItem> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<ProcurementOrderItem> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new ProcurementOrderItem());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent ProcurementOrderItem model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new ProcurementOrderItem(), displayedEntity);
	}
