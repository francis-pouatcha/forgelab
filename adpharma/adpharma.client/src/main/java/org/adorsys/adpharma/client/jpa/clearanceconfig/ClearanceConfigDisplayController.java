...
@Singleton
public class ClearanceConfigDisplayController implements EntityController
{

   @Inject
   private ClearanceConfigDisplayView displayView;
   ...
   private Event<ClearanceConfig> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<ClearanceConfig> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<ClearanceConfig>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<ClearanceConfig>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<ClearanceConfig>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private ClearanceConfig displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<ClearanceConfig> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<ClearanceConfig> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(ClearanceConfig.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new ClearanceConfig());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent ClearanceConfig model)
   {
