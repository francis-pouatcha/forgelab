...
public class SalesOrderRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<SalesOrder> removedEvent;

   @Inject
   private SalesOrderRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent SalesOrder entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent SalesOrder entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("SalesOrder_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SalesOrderRemoveService service = (SalesOrderRemoveService) wse
                  .getSource();
            SalesOrder p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
