...
public abstract class CashOutAgencyController {
	 @Inject
	   ...
	   @Inject
	   @Bundle({ CrudKeys.class, CashOut.class, Agency.class })
	   private ResourceBundle resourceBundle;

	   @Inject
	   private ErrorMessageDialog errorMessageDialog;

	   protected CashOut sourceEntity;

	   protected void disableButton(final CashOutAgencySelection selection)
	   {
	      selection.getAgency().setDisable(true);
	   }

	   protected void activateButton(final CashOutAgencySelection selection)
	   {
	   }

	   protected void bind(final CashOutAgencySelection selection, final CashOutAgencyForm form)
	   {
	      ...
	      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
	      {
	         ...
	         public void handle(WorkerStateEvent event)
	         {
	            ...
	            selection.getAgency().getItems().add(new CashOutAgency());
	            for (Agency entity : entities)
	            {
	               selection.getAgency().getItems().add(new CashOutAgency(entity));
	            }
	         }
	      });
	      ...
	      selection.getAgency().valueProperty().addListener(new ChangeListener<CashOutAgency>()
	      {
	         @Override
	         public void changed(ObservableValue<? extends CashOutAgency> ov, CashOutAgency oldValue,
	        		 CashOutAgency newValue)
	         {
