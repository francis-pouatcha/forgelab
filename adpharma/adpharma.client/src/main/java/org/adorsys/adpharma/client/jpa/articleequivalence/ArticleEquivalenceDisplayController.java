...
@Singleton
public class ArticleEquivalenceDisplayController implements EntityController
{
   ...
   private Event<ArticleEquivalence> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<ArticleEquivalence> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<ArticleEquivalence>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<ArticleEquivalence>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<ArticleEquivalence>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private ArticleEquivalence displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<ArticleEquivalence> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<ArticleEquivalence> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new ArticleEquivalence());
   }
