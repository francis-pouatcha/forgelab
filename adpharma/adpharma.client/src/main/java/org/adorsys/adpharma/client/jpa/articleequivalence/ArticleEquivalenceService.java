...
public class ArticleEquivalenceService
{
   ...
   public ArticleEquivalenceService()
   {
      ...
      this.target = client.target(serverAddress + "/rest/articleequivalences");
   }
   ...
   {//@PathParam("id")
      // TODO encode id
      return target.path("" + id).request(media).delete(ArticleEquivalence.class);
   }
   ...
   public ArticleEquivalence update(ArticleEquivalence entity)
   {
      ...
      return target.path("" + entity.getId())
            .request(media).put(ent, ArticleEquivalence.class);
   }
   ...
   {// @PathParam("id") 
      return target.path("" + id).request(media).get(ArticleEquivalence.class);
   }
   ...
   {
      return target.request(media).get(ArticleEquivalenceSearchResult.class);
   }
   ...
   {
      Entity<ArticleEquivalenceSearchInput> searchInputEntity = Entity.entity(
            searchInput, media);
      return target.path(FIND_BY).request(media).post(
            searchInputEntity, ArticleEquivalenceSearchResult.class);
   }
