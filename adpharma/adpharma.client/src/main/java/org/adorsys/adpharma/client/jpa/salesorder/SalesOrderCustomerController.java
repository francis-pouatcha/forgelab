...
public abstract class SalesOrderCustomerController
{
   ...
   private Event<SalesOrderCustomerSelectionEventData> selectionRequestEvent;

   private SalesOrderCustomerSelectionEventData pendingSelectionRequest;
   ...
   protected void disableButton(final SalesOrderCustomerSelection selection, final SalesOrderCustomerForm form)
   {
      selection.getSelectButton().setDisable(true);
   }

   protected void activateButton(final SalesOrderCustomerSelection selection, final SalesOrderCustomerForm form)
   {
   }

   protected void bind(final SalesOrderCustomerSelection selection, final SalesOrderCustomerForm form)
   {
      selection.getSelectButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               {
                  pendingSelectionRequest = new SalesOrderCustomerSelectionEventData(
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            event.consume();
            s.reset();
            sourceEntity.setCustomer(new SalesOrderCustomer(entity));
         }
      });
   ...
   public void handleAssocSelectionResponseEvent(@Observes @AssocSelectionResponseEvent SalesOrderCustomerSelectionEventData eventData)
   {
            ...
            && eventData.getSourceEntity() == sourceEntity && eventData.getTargetEntity() != null)
      {
         if (sourceEntity != null)
            sourceEntity.setCustomer(new SalesOrderCustomer(eventData.getTargetEntity()));
      }
