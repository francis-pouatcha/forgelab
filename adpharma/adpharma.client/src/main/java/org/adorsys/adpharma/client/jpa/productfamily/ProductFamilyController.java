...
@Singleton
public class ProductFamilyController extends DomainComponentController
{

   @Inject
   private ProductFamilySearchController searchController;

   @Inject
   private ProductFamilyCreateController createController;

   @Inject
   private ProductFamilyEditController editController;

   @Inject
   private ProductFamilyListController listController;

   @Inject
   private ProductFamilyDisplayController displayController;

   @Inject
   private ProductFamilyIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<ProductFamily> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<ProductFamily> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<ProductFamily> selectedModelEvent;

   private ProductFamily createModel = new ProductFamily();
   private ProductFamily searchModel = new ProductFamily();
   private ProductFamily selectedModel = new ProductFamily();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<ProductFamily> entities)
   {
   ...
   public void handleSelectionEvent(@Observes @EntitySelectionEvent ProductFamily selectedEntity)
   {
      if (!registration.canRead())
         return;
      // if result is empty: display no result.
      // else display list of productFamilys.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent ProductFamily selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
   ...
   /**
    * Display search form.
    * @param selectedProductFamily
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent ProductFamily templateEntity)
   {
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent ProductFamily selectedEntity)
   {
