...
public class CustomerCategoryRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<CustomerCategory> removedEvent;

   @Inject
   private CustomerCategoryRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent CustomerCategory entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent CustomerCategory entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("CustomerCategory_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerCategoryRemoveService service = (CustomerCategoryRemoveService) wse
                  .getSource();
            CustomerCategory p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
