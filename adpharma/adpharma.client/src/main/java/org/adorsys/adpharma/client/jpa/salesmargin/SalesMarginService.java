...
public class SalesMarginService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/salesmargins").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public SalesMargin deleteById(Long id) {// @PathParam("id")
											// TODO encode id
	...
	public SalesMargin update(SalesMargin entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, SalesMargin.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public SalesMargin findById(Long id) {// @PathParam("id")
	...
	public SalesMarginSearchResult findBy(SalesMarginSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, SalesMarginSearchResult.class);
	}
