...
@Singleton
public class CustomerCategoryCreateController implements EntityController
{

   @Inject
   private CustomerCategoryCreateView createView;

   @Inject
   private CustomerCategoryCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<CustomerCategory> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getResetButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         {
            PropertyReader.copy(new CustomerCategory(), model);
         }
      });
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<CustomerCategory>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerCategoryCreateService s = (CustomerCategoryCreateService) event.getSource();
            CustomerCategory ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent CustomerCategory model)
   {
      this.model = model;
