...
public class ArticleEquivalenceRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<ArticleEquivalence> removedEvent;

   @Inject
   private ArticleEquivalenceRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent ArticleEquivalence entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent ArticleEquivalence entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("ArticleEquivalence_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ArticleEquivalenceRemoveService service = (ArticleEquivalenceRemoveService) wse
                  .getSource();
            ArticleEquivalence p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
