...
@Singleton
public class VATEditController implements EntityController
{

   @Inject
   private VATEditView editView;

   @Inject
   private VATEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private VATLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<VAT> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<VAT> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<VAT>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setVAT(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            VATEditService s = (VATEditService) event.getSource();
            VAT entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            VATLoadService s = (VATLoadService) event.getSource();
            VAT entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent VAT p)
   {
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent VAT model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new VAT(), displayedEntity);
	}
