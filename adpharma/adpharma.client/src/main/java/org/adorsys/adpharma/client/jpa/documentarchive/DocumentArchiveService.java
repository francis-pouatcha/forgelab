...
public class DocumentArchiveService
{
   ...
   public DocumentArchiveService()
   {
      ...
      this.target = client.target(serverAddress + "/rest/documentarchives");
   }
   ...
   {//@PathParam("id")
      // TODO encode id
      return target.path("" + id).request(media).delete(DocumentArchive.class);
   }
   ...
   public DocumentArchive update(DocumentArchive entity)
   {
      ...
      return target.path("" + entity.getId())
            .request(media).put(ent, DocumentArchive.class);
   }
   ...
   public DocumentArchiveSearchResult findBy(DocumentArchiveSearchInput searchInput)
   {
      ...
      return target.path(FIND_BY).request(media).post(
            searchInputEntity, DocumentArchiveSearchResult.class);
   }
