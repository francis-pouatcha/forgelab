...
@Singleton
public class DebtStatementEditController implements EntityController
{

   @Inject
   private DebtStatementEditView editView;

   @Inject
   private DebtStatementEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private DebtStatementLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<DebtStatement> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<DebtStatement> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<DebtStatement>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setDebtStatement(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            DebtStatement entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            DebtStatementLoadService s = (DebtStatementLoadService) event.getSource();
            DebtStatement entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent DebtStatement model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new DebtStatement(), displayedEntity);
	}
