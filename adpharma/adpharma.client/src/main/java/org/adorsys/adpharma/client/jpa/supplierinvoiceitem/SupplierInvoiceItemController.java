...
@Singleton
public class SupplierInvoiceItemController extends DomainComponentController
{

   @Inject
   private SupplierInvoiceItemSearchController searchController;

   @Inject
   private SupplierInvoiceItemCreateController createController;

   @Inject
   private SupplierInvoiceItemEditController editController;

   @Inject
   private SupplierInvoiceItemListController listController;

   @Inject
   private SupplierInvoiceItemDisplayController displayController;

   @Inject
   private SupplierInvoiceItemIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<SupplierInvoiceItem> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<SupplierInvoiceItem> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<SupplierInvoiceItem> selectedModelEvent;

   private SupplierInvoiceItem createModel = new SupplierInvoiceItem();
   private SupplierInvoiceItem searchModel = new SupplierInvoiceItem();
   private SupplierInvoiceItem selectedModel = new SupplierInvoiceItem();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<SupplierInvoiceItem> entities)
   {
    ...
    */
   public void handleSelectionEvent(@Observes @EntitySelectionEvent SupplierInvoiceItem selectedEntity)
   {
      if (!registration.canRead())
         return;
      // if result is empty: display no result.
      // else display list of supplierInvoiceItems.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent SupplierInvoiceItem selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
   ...
   /**
    * Display search form.
    * @param selectedSupplierInvoiceItem
    */
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent SupplierInvoiceItem selectedEntity)
   {
