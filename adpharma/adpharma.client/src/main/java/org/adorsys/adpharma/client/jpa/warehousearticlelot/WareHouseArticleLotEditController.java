...
@Singleton
public class WareHouseArticleLotEditController implements EntityController
{

   @Inject
   private WareHouseArticleLotEditView editView;

   @Inject
   private WareHouseArticleLotEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private WareHouseArticleLotLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<WareHouseArticleLot> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<WareHouseArticleLot> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<WareHouseArticleLot>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setWareHouse(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            WareHouseArticleLotEditService s = (WareHouseArticleLotEditService) event.getSource();
            WareHouseArticleLot entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            WareHouseArticleLotLoadService s = (WareHouseArticleLotLoadService) event.getSource();
            WareHouseArticleLot entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent WareHouseArticleLot p)
   {
