...
@Singleton
public class SalesOrderEditController implements EntityController
{

   @Inject
   private SalesOrderEditView editView;

   @Inject
   private SalesOrderEditService editService;
   @Inject
   ...
   private Event<SalesOrder> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<SalesOrder> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<SalesOrder>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setSalesOrder(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SalesOrderEditService s = (SalesOrderEditService) event.getSource();
