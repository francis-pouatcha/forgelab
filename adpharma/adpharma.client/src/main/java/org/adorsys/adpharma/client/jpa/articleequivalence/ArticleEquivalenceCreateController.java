...
@Singleton
public class ArticleEquivalenceCreateController implements EntityController
{

   @Inject
   private ArticleEquivalenceCreateView createView;

   @Inject
   private ArticleEquivalenceCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<ArticleEquivalence> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<ArticleEquivalence>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ArticleEquivalenceCreateService s = (ArticleEquivalenceCreateService) event.getSource();
            ArticleEquivalence ent = s.getValue();
            event.consume();
            s.reset();
