...
@Singleton
public class SectionEditController implements EntityController
{

   @Inject
   private SectionEditView editView;

   @Inject
   private SectionEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private SectionLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<Section> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<Section> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SectionEditService s = (SectionEditService) event.getSource();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            SectionLoadService s = (SectionLoadService) event.getSource();
            Section entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent Section p)
   {
