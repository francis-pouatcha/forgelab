...
public class ArticleLotRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<ArticleLot> removedEvent;

   @Inject
   private ArticleLotRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent ArticleLot entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent ArticleLot entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("ArticleLot_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ArticleLotRemoveService service = (ArticleLotRemoveService) wse
                  .getSource();
            ArticleLot p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
