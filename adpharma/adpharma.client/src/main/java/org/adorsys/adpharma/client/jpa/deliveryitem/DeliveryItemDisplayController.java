...
public class DeliveryItemDisplayController implements EntityController
{
   ...
   private Event<DeliveryItem> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<DeliveryItem> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<DeliveryItem>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<DeliveryItem>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<DeliveryItem>>();
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<DeliveryItem> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<DeliveryItem> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new DeliveryItem());
   }
