...
public class CustomerVoucherRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<CustomerVoucher> removedEvent;

   @Inject
   private CustomerVoucherRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent CustomerVoucher entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent CustomerVoucher entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("CustomerVoucher_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerVoucherRemoveService service = (CustomerVoucherRemoveService) wse
                  .getSource();
            CustomerVoucher p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
