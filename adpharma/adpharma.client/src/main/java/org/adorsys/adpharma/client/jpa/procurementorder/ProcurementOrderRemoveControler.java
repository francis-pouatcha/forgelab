...
public class ProcurementOrderRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<ProcurementOrder> removedEvent;

   @Inject
   private ProcurementOrderRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent ProcurementOrder entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent ProcurementOrder entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("ProcurementOrder_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProcurementOrderRemoveService service = (ProcurementOrderRemoveService) wse
                  .getSource();
            ProcurementOrder p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
