...
@Singleton
public class CustomerInvoiceCreateController implements EntityController
{

   @Inject
   private CustomerInvoiceCreateView createView;

   @Inject
   private CustomerInvoiceCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<CustomerInvoice> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<CustomerInvoice>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerInvoiceCreateService s = (CustomerInvoiceCreateService) event.getSource();
            CustomerInvoice ent = s.getValue();
            event.consume();
            s.reset();
