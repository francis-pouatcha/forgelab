...
@Singleton
public class CashOutCreateController implements EntityController{
	 @Inject
	   private CashOutCreateView createView;

	   @Inject
	   private CashOutProcessService createService;
	   @Inject
	   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

	   @Inject
	   @EntityCreateDoneEvent
	   private Event<CashOut> createDoneEvent;

	   @Inject
	   @EntitySearchRequestedEvent
	   private Event<CashOut> searchRequestedEvent;
	   ...
	   public void init()
	   {
	      ...
	      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
	      {
	         ...
	         public void handle(ActionEvent e)
	         {
	            Set<ConstraintViolation<CashOut>> violations = createView.getView().validate(model);
	    ...
	    * @param createdEntity
	    */
	   public void handleCreatedEvent(@Observes @EntityCreateDoneEvent CashOut createdEntity)
	   {
	      PropertyReader.copy(new CashOut(), model);
	   }
