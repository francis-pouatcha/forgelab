...
@Singleton
public class SupplierInvoiceCreateController implements EntityController
{

   @Inject
   private SupplierInvoiceCreateView createView;

   @Inject
   private SupplierInvoiceCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<SupplierInvoice> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<SupplierInvoice>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceCreateService s = (SupplierInvoiceCreateService) event.getSource();
            SupplierInvoice ent = s.getValue();
            event.consume();
            s.reset();
