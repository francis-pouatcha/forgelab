...
@Singleton
public class CustomerVoucherDisplayController implements EntityController
{
   ...
   private Event<CustomerVoucher> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<CustomerVoucher> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<CustomerVoucher>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<CustomerVoucher>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<CustomerVoucher>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private CustomerVoucher displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<CustomerVoucher> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<CustomerVoucher> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new CustomerVoucher());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent CustomerVoucher model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new CustomerVoucher(), displayedEntity);
	}
}