...
@Singleton
public class HospitalDisplayController implements EntityController
{

   @Inject
   private HospitalDisplayView displayView;
   ...
   private Event<Hospital> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<Hospital> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<Hospital>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<Hospital>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<Hospital>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private Hospital displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<Hospital> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<Hospital> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(Hospital.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new Hospital());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent Hospital model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new Hospital(), displayedEntity);
	}
}