...
@Singleton
public class ProductFamilyCreateController implements EntityController
{

   @Inject
   private ProductFamilyCreateView createView;

   @Inject
   private ProductFamilyCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<ProductFamily> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<ProductFamily>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProductFamilyCreateService s = (ProductFamilyCreateService) event.getSource();
            ProductFamily ent = s.getValue();
            event.consume();
            s.reset();
