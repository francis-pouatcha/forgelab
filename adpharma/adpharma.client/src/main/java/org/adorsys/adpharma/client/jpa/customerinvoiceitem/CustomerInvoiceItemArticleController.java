...
public abstract class CustomerInvoiceItemArticleController
{
   ...
   protected void disableButton(final CustomerInvoiceItemArticleSelection selection)
   {
      selection.getArticle().setDisable(true);
   }

   protected void activateButton(final CustomerInvoiceItemArticleSelection selection)
   {
   }

   protected void bind(final CustomerInvoiceItemArticleSelection selection)
   {
      ...
      selection.getArticle().valueProperty().addListener(new ChangeListener<Article>()
      {
               ...
               Article newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setArticle(new CustomerInvoiceItemArticle(newValue));
         }
      });
