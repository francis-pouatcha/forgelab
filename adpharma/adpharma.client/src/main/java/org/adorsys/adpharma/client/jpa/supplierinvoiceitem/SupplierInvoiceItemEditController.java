...
@Singleton
public class SupplierInvoiceItemEditController implements EntityController
{

   @Inject
   private SupplierInvoiceItemEditView editView;

   @Inject
   private SupplierInvoiceItemEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private SupplierInvoiceItemLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<SupplierInvoiceItem> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<SupplierInvoiceItem> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<SupplierInvoiceItem>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setSupplierInvoiceItem(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceItemEditService s = (SupplierInvoiceItemEditService) event.getSource();
            SupplierInvoiceItem entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            SupplierInvoiceItemLoadService s = (SupplierInvoiceItemLoadService) event.getSource();
            SupplierInvoiceItem entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent SupplierInvoiceItem p)
   {
