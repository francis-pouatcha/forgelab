...
public class DebtStatementService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/debtstatements").register(
				clientCookieFilter);
	}
	...
	public DebtStatement update(DebtStatement entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, DebtStatement.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public DebtStatement findById(Long id) {// @PathParam("id")
	...
	// @Produces("application/xml")
	// @Consumes("application/xml")
	public DebtStatementSearchResult findByLike(
