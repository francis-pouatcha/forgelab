...
public class ProcurementReportPrintTemplatePDF {
...
private PeriodicalDeliveryDataSearchInput model;
	...
	public ProcurementReportPrintTemplatePDF(
			Login login,LoginAgency agency,PeriodicalDeliveryDataSearchInput model
			) throws DocumentException {
		this.agency = agency;
		this.login = login;
		this.model = model;
		PropertyReader.copy(login.getAgency(), agency);
		pdfFileName = "procurement.pdf";
	...
	public void addItems(List<DeliveryItem> items) {
		...
		for (DeliveryItem item : items) {
			...
			totalQty = totalQty.add(item.getQtyOrdered());
			totalPrice = totalPrice.add(item.getTotalPurchasePrice());
	...
	private void fillTableHaeder() throws DocumentException {
		...
		pdfPCell = new PdfPCell();
		pdfPCell.addElement(new StandardText("Qte Livree "));
		reportTable.addCell(pdfPCell);
	...
	private void printReportHeader() throws DocumentException {
		...
		Paragraph paragraph = new Paragraph(new BoldText("RAPPORTS PERIODIQUE DES LIVRAISONS"));
		...
		paragraph = new Paragraph(new StandardText("Fournisseur : "+model.getSupplier()));
		paragraph.setAlignment(Element.ALIGN_LEFT);
		document.add(paragraph);
