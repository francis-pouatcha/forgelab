...
@Singleton
public class PermissionNameEditController implements EntityController
{

   @Inject
   private PermissionNameEditView editView;

   @Inject
   private PermissionNameEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private PermissionNameLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<PermissionName> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<PermissionName> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<PermissionName>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setPermissionName(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PermissionNameEditService s = (PermissionNameEditService) event.getSource();
            PermissionName entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            PermissionNameLoadService s = (PermissionNameLoadService) event.getSource();
            PermissionName entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent PermissionName p)
   {
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent PermissionName model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new PermissionName(), displayedEntity);
	}
