...
@Singleton
public class InventoryItemEditController implements EntityController
{

   @Inject
   private InventoryItemEditView editView;

   @Inject
   private InventoryItemEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private InventoryItemLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<InventoryItem> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<InventoryItem> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<InventoryItem>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setInventoryItem(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            InventoryItem entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            InventoryItemLoadService s = (InventoryItemLoadService) event.getSource();
            InventoryItem entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent InventoryItem model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new InventoryItem(), displayedEntity);
	}
