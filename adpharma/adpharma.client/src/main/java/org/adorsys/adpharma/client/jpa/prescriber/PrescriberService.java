...
public class PrescriberService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/prescribers").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public Prescriber deleteById(Long id) {// @PathParam("id")
											// TODO encode id
	...
	public Prescriber update(Prescriber entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, Prescriber.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public Prescriber findById(Long id) {// @PathParam("id")
	...
	public PrescriberSearchResult findBy(PrescriberSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, PrescriberSearchResult.class);
	}
