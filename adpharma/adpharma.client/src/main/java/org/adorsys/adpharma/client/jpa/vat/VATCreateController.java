...
@Singleton
public class VATCreateController implements EntityController
{

   @Inject
   private VATCreateView createView;

   @Inject
   private VATCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<VAT> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<VAT>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            VATCreateService s = (VATCreateService) event.getSource();
            VAT ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent VAT model)
   {
      this.model = model;
