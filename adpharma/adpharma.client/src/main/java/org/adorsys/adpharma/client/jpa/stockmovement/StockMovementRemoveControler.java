...
public class StockMovementRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<StockMovement> removedEvent;

   @Inject
   private StockMovementRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent StockMovement entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent StockMovement entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("StockMovement_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            StockMovementRemoveService service = (StockMovementRemoveService) wse
                  .getSource();
            StockMovement p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
