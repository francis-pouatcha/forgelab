...
public class DeliveryRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Delivery> removedEvent;

   @Inject
   private DeliveryRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Delivery entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Delivery entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Delivery_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DeliveryRemoveService service = (DeliveryRemoveService) wse
                  .getSource();
            Delivery p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
