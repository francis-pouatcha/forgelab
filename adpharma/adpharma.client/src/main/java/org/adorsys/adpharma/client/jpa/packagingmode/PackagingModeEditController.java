...
@Singleton
public class PackagingModeEditController implements EntityController
{

   @Inject
   private PackagingModeEditView editView;

   @Inject
   private PackagingModeEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private PackagingModeLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<PackagingMode> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<PackagingMode> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<PackagingMode>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setPackagingMode(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PackagingModeEditService s = (PackagingModeEditService) event.getSource();
            PackagingMode entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            PackagingModeLoadService s = (PackagingModeLoadService) event.getSource();
            PackagingMode entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent PackagingMode p)
   {
