...
public class InventoryView extends AbstractForm<Inventory>
{
	...
	@Inject
	private InventorySectionForm inventorySectionForm;
	@Inject
	private InventorySectionSelection inventorySectionSelection;
	...
	public void postConstruct()
	{
		...
		inventoryNumber = viewBuilder.addTextField("Inventory_inventoryNumber_description.title", "inventoryNumber", resourceBundle,ViewModel.READ_ONLY);
		description = viewBuilder.addTextArea("Inventory_description_description.title", "description", resourceBundle);
		inventoryStatus = viewBuilder.addComboBox("Inventory_inventoryStatus_description.title", "inventoryStatus", resourceBundle, DocumentProcessingState.values(),ViewModel.READ_ONLY);
		gapSaleAmount = viewBuilder.addBigDecimalField("Inventory_gapSaleAmount_description.title", "gapSaleAmount", resourceBundle, NumberType.CURRENCY, locale,ViewModel.READ_ONLY);
		gapPurchaseAmount = viewBuilder.addBigDecimalField("Inventory_gapPurchaseAmount_description.title", "gapPurchaseAmount", resourceBundle, NumberType.CURRENCY, locale,ViewModel.READ_ONLY);
		inventoryDate = viewBuilder.addCalendarTextField("Inventory_inventoryDate_description.title", "inventoryDate", resourceBundle, "dd-MM-yyyy HH:mm", locale,ViewModel.READ_ONLY);
		...
		viewBuilder.addSubForm("Inventory_recordingUser_description.title", "recordingUser", resourceBundle, inventoryRecordingUserSelection, ViewModel.READ_ONLY);
		...
		viewBuilder.addSubForm("Inventory_section_description.title", "section", resourceBundle, inventorySectionSelection, ViewModel.READ_WRITE);
		createEmpty = viewBuilder.addCheckBox("Entity_empty.title", "createEmpty", resourceBundle);
		createEmpty.setText(resourceBundle.getString("Inventory_createEmpty_description.title"));
	...
	public void bind(Inventory model)
	{
		...
		inventorySectionSelection.bind(model);
		createEmpty.selectedProperty().bindBidirectional(model.createEmptyProperty());
	}
	...
	public CheckBox getCreateEmpty() {
		return createEmpty;
	}

	public InventorySectionForm getInventorySectionForm() {
		return inventorySectionForm;
	}

	public InventorySectionSelection getInventorySectionSelection() {
		return inventorySectionSelection;
	}
	...
	public ResourceBundle getInventoryStatusBundle() {
		return inventoryStatusBundle;
	}

	public DocumentProcessingStateConverter getInventoryStatusConverter() {
		return inventoryStatusConverter;
	}

	public DocumentProcessingStateListCellFatory getInventoryStatusListCellFatory() {
		return inventoryStatusListCellFatory;
	}
