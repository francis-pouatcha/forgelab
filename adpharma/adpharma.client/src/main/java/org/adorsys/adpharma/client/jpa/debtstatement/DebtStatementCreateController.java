...
@Singleton
public class DebtStatementCreateController implements EntityController
{

   @Inject
   private DebtStatementCreateView createView;

   @Inject
   private DebtStatementCreateService createService;
   @Inject
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<DebtStatement>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DebtStatementCreateService s = (DebtStatementCreateService) event.getSource();
            DebtStatement ent = s.getValue();
            event.consume();
            s.reset();
