...
@Singleton
public class ProcurementOrderItemCreateController implements EntityController
{

   @Inject
   private ProcurementOrderItemCreateView createView;

   @Inject
   private ProcurementOrderItemCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<ProcurementOrderItem> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<ProcurementOrderItem>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            ProcurementOrderItem ent = s.getValue();
            event.consume();
            s.reset();
