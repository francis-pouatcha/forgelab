...
@Singleton
public class AgencyCreateController implements EntityController
{

   @Inject
   private AgencyCreateView createView;

   @Inject
   private AgencyCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Agency> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Agency>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            AgencyCreateService s = (AgencyCreateService) event.getSource();
            Agency ent = s.getValue();
            event.consume();
            s.reset();
