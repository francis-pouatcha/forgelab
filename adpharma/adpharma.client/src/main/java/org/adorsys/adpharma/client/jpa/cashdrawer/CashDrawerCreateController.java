...
@Singleton
public class CashDrawerCreateController implements EntityController
{

   @Inject
   private CashDrawerCreateView createView;

   @Inject
   private CashDrawerCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<CashDrawer> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<CashDrawer>> violations = createView.getView().validate(model);
