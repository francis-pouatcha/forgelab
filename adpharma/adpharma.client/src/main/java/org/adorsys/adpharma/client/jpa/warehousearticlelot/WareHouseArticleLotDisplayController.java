...
@Singleton
public class WareHouseArticleLotDisplayController implements EntityController
{

   @Inject
   private WareHouseArticleLotDisplayView displayView;
   ...
   private Event<WareHouseArticleLot> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<WareHouseArticleLot> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<WareHouseArticleLot>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<WareHouseArticleLot>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<WareHouseArticleLot>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private WareHouseArticleLot displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<WareHouseArticleLot> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<WareHouseArticleLot> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(WareHouseArticleLot.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new WareHouseArticleLot());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent WareHouseArticleLot model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new WareHouseArticleLot(), displayedEntity);
	}
}