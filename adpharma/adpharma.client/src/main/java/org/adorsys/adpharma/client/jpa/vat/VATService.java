...
public class VATService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/vats").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public VAT deleteById(Long id) {// @PathParam("id")
									// TODO encode id
	...
	public VAT findById(Long id) {// @PathParam("id")
	...
	public VATSearchResult findBy(VATSearchInput searchInput) {
		Entity<VATSearchInput> searchInputEntity = Entity.entity(searchInput,
				media);
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, VATSearchResult.class);
	}
