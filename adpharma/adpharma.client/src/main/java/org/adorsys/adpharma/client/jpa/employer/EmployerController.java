...
public class EmployerController extends DomainComponentController
{
   ...
   @Inject
   private EmployerSearchController searchController;

   @Inject
   private EmployerCreateController createController;

   @Inject
   private EmployerEditController editController;

   @Inject
   private EmployerListController listController;

   @Inject
   private EmployerDisplayController displayController;

   @Inject
   private EmployerIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Employer> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Employer> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Employer> selectedModelEvent;

   private Employer createModel = new Employer();
   private Employer searchModel = new Employer();
   private Employer selectedModel = new Employer();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Employer> entities)
   {
    ...
    */
   public void handleSelectionEvent(@Observes @EntitySelectionEvent Employer selectedEntity)
   {
      if (!registration.canRead())
         return;
      // if result is empty: display no result.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Employer selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
    ...
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent Employer templateEntity)
   {
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Employer selectedEntity)
   {
