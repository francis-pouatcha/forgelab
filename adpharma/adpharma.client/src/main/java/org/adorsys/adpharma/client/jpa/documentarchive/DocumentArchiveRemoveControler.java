...
public class DocumentArchiveRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<DocumentArchive> removedEvent;

   @Inject
   private DocumentArchiveRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent DocumentArchive entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent DocumentArchive entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("DocumentArchive_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DocumentArchiveRemoveService service = (DocumentArchiveRemoveService) wse
                  .getSource();
            DocumentArchive p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
