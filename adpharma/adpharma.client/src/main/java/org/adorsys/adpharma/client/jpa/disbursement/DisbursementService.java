...
public class DisbursementService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/disbursements").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public Disbursement deleteById(Long id) {// @PathParam("id")
												// TODO encode id
	...
	public Disbursement findById(Long id) {// @PathParam("id")
	...
	public DisbursementSearchResult findBy(DisbursementSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, DisbursementSearchResult.class);
	}
	...
	// @Produces("application/xml")
	// @Consumes("application/xml")
	public DisbursementSearchResult findByLike(
