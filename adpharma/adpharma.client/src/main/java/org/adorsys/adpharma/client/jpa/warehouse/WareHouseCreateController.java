...
@Singleton
public class WareHouseCreateController implements EntityController
{

   @Inject
   private WareHouseCreateView createView;

   @Inject
   private WareHouseCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<WareHouse> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<WareHouse>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            WareHouseCreateService s = (WareHouseCreateService) event.getSource();
            WareHouse ent = s.getValue();
            event.consume();
            s.reset();
