...
public class ArticleLotEditController implements EntityController
{
   ...
   @Inject
   private ArticleLotEditView editView;

   @Inject
   private ArticleLotEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ArticleLotLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ArticleLot> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ArticleLot> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ArticleLotEditService s = (ArticleLotEditService) event.getSource();
            ArticleLot entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ArticleLotLoadService s = (ArticleLotLoadService) event.getSource();
            ArticleLot entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
