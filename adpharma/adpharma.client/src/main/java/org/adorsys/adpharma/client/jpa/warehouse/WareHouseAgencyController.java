...
public abstract class WareHouseAgencyController
{
   @Inject
   ...
   protected WareHouse sourceEntity;

   protected void disableButton(final WareHouseAgencySelection selection)
   {
      selection.getAgency().setDisable(true);
   }

   protected void activateButton(final WareHouseAgencySelection selection)
   {
   }

   protected void bind(final WareHouseAgencySelection selection, final WareHouseAgencyForm form)
   {

      //	    selection.getAgency().valueProperty().bindBidirectional(sourceEntity.recordingUserProperty());
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            selection.getAgency().getItems().add(new WareHouseAgency());
            for (Agency entity : entities)
            {
               selection.getAgency().getItems().add(new WareHouseAgency(entity));
            }
         }
      });
      ...
      selection.getAgency().valueProperty().addListener(new ChangeListener<WareHouseAgency>()
      {
         @Override
         public void changed(ObservableValue<? extends WareHouseAgency> ov, WareHouseAgency oldValue,
               WareHouseAgency newValue)
         {
