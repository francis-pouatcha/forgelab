...
public abstract class CustomerInvoiceCustomerController
{
   ...
   protected void disableButton(final CustomerInvoiceCustomerSelection selection)
   {
      selection.getCustomer().setDisable(true);
   }

   protected void activateButton(final CustomerInvoiceCustomerSelection selection)
   {
   }

   protected void bind(final CustomerInvoiceCustomerSelection selection)
   {
      ...
      selection.getCustomer().valueProperty().addListener(new ChangeListener<Customer>()
      {
               ...
               Customer newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setCustomer(new CustomerInvoiceCustomer(newValue));
         }
      });
