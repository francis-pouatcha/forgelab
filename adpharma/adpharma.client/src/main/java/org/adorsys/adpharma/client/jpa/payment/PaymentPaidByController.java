...
public abstract class PaymentPaidByController
{
   ...
   private Event<PaymentPaidBySelectionEventData> selectionRequestEvent;

   private PaymentPaidBySelectionEventData pendingSelectionRequest;
   ...
   protected void disableButton(final PaymentPaidBySelection selection, final PaymentPaidByForm form)
   {
      selection.getSelectButton().setDisable(true);
   }

   protected void activateButton(final PaymentPaidBySelection selection, final PaymentPaidByForm form)
   {
   }

   protected void bind(final PaymentPaidBySelection selection, final PaymentPaidByForm form)
   {
      selection.getSelectButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               {
                  pendingSelectionRequest = new PaymentPaidBySelectionEventData(
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            event.consume();
            s.reset();
            sourceEntity.setPaidBy(new PaymentPaidBy(entity));
         }
      });
   ...
   public void handleAssocSelectionResponseEvent(@Observes @AssocSelectionResponseEvent PaymentPaidBySelectionEventData eventData)
   {
            ...
            && eventData.getSourceEntity() == sourceEntity && eventData.getTargetEntity() != null)
      {
         if (sourceEntity != null)
            sourceEntity.setPaidBy(new PaymentPaidBy(eventData.getTargetEntity()));
      }
