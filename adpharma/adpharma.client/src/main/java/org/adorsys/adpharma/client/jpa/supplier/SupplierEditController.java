...
@Singleton
public class SupplierEditController implements EntityController
{

   @Inject
   private SupplierEditView editView;

   @Inject
   private SupplierEditService editService;
   @Inject
   ...
   private Event<Supplier> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<Supplier> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<Supplier>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setSupplier(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierEditService s = (SupplierEditService) event.getSource();
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent Supplier p)
   {
