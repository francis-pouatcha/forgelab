...
@Singleton
public class CurrencyDisplayController implements EntityController
{

   @Inject
   private CurrencyDisplayView displayView;
   ...
   private Event<Currency> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<Currency> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<Currency>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<Currency>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<Currency>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private Currency displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<Currency> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<Currency> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(Currency.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new Currency());
   }
    ...
    * @param model
    */
   public void handleNewModelEvent(@Observes @SelectedModelEvent Currency model)
   {
