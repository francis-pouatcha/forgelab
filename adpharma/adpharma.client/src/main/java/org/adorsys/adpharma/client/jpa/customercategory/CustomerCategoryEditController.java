...
@Singleton
public class CustomerCategoryEditController implements EntityController
{

   @Inject
   private CustomerCategoryEditView editView;

   @Inject
   private CustomerCategoryEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private CustomerCategoryLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<CustomerCategory> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<CustomerCategory> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<CustomerCategory>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setCustomerCategory(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerCategoryEditService s = (CustomerCategoryEditService) event.getSource();
            CustomerCategory entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            CustomerCategoryLoadService s = (CustomerCategoryLoadService) event.getSource();
            CustomerCategory entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent CustomerCategory p)
   {
