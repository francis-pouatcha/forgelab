...
@Singleton
public class PackagingModeCreateController implements EntityController
{

   @Inject
   private PackagingModeCreateView createView;

   @Inject
   private PackagingModeCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<PackagingMode> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<PackagingMode>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            PackagingModeCreateService s = (PackagingModeCreateService) event.getSource();
            PackagingMode ent = s.getValue();
            event.consume();
            s.reset();
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent PackagingMode model)
   {
      this.model = model;
