...
@Singleton
public class CustomerVoucherCreateController implements EntityController
{

   @Inject
   private CustomerVoucherCreateView createView;

   @Inject
   private CustomerVoucherCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<CustomerVoucher> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<CustomerVoucher>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerVoucherCreateService s = (CustomerVoucherCreateService) event.getSource();
            CustomerVoucher ent = s.getValue();
            event.consume();
            s.reset();
