...
@Singleton
public class CustomerVoucherEditController implements EntityController
{

   @Inject
   private CustomerVoucherEditView editView;

   @Inject
   private CustomerVoucherEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private CustomerVoucherLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<CustomerVoucher> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<CustomerVoucher> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<CustomerVoucher>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setCustomerVoucher(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerVoucherEditService s = (CustomerVoucherEditService) event.getSource();
            CustomerVoucher entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            CustomerVoucherLoadService s = (CustomerVoucherLoadService) event.getSource();
            CustomerVoucher entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
