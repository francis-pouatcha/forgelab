...
public class ProductFamilyRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<ProductFamily> removedEvent;

   @Inject
   private ProductFamilyRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent ProductFamily entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent ProductFamily entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("ProductFamily_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProductFamilyRemoveService service = (ProductFamilyRemoveService) wse
                  .getSource();
            ProductFamily p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
