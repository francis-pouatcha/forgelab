...
@Singleton
public class SupplierController extends DomainComponentController
{

   @Inject
   private SupplierSearchController searchController;

   @Inject
   private SupplierCreateController createController;

   @Inject
   private SupplierEditController editController;

   @Inject
   private SupplierListController listController;

   @Inject
   private SupplierDisplayController displayController;

   @Inject
   private SupplierIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Supplier> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Supplier> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Supplier> selectedModelEvent;

   private Supplier createModel = new Supplier();
   private Supplier searchModel = new Supplier();
   private Supplier selectedModel = new Supplier();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Supplier> entities)
   {
    ...
    */
   public void handleSelectionEvent(@Observes @EntitySelectionEvent Supplier selectedEntity)
   {
      // if result is empty: display no result.
      // else display list of suppliers.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Supplier selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
    ...
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent Supplier templateEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Supplier selectedEntity)
   {
      List<EntityController> displayedViews = getDisplayedViews();
      displayedViews.clear();
      displayedViews.add(listController);
