...
@Singleton
public class ProductDetailConfigDisplayController implements EntityController
{
   ...
   private Event<ProductDetailConfig> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<ProductDetailConfig> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<ProductDetailConfig>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<ProductDetailConfig>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<ProductDetailConfig>>();
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<ProductDetailConfig> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<ProductDetailConfig> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new ProductDetailConfig());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent ProductDetailConfig model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new ProductDetailConfig(), displayedEntity);
	}
}