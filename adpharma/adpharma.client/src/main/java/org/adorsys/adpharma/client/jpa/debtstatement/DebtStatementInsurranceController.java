...
public abstract class DebtStatementInsurranceController
{
   ...
   protected void disableButton(final DebtStatementInsurranceSelection selection)
   {
      selection.getInsurrance().setDisable(true);
   }

   protected void activateButton(final DebtStatementInsurranceSelection selection)
   {
   }

   protected void bind(final DebtStatementInsurranceSelection selection)
   {
      ...
      searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            selection.getInsurrance().getItems().clear();
            selection.getInsurrance().getItems().addAll(entities);
         }
      });
      ...
      selection.getInsurrance().valueProperty().addListener(new ChangeListener<Customer>()
      {
         @Override
               ...
               Customer newValue)
         {
            if (sourceEntity != null)
               sourceEntity.setInsurrance(new DebtStatementInsurrance(newValue));
         }
      });

      selection.getInsurrance().armedProperty().addListener(new ChangeListener<Boolean>()
      {
