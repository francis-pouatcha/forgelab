...
@Singleton
public class HospitalCreateController implements EntityController
{

   @Inject
   private HospitalCreateView createView;

   @Inject
   private HospitalCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Hospital> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Hospital>> violations = createView.getView().validate(model);
   ...
   public void handleNewModelEvent(@Observes @CreateModelEvent Hospital model)
   {
      this.model = model;
