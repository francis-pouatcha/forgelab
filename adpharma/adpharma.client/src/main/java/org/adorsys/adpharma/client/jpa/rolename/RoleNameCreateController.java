...
@Singleton
public class RoleNameCreateController implements EntityController
{

   @Inject
   private RoleNameCreateView createView;

   @Inject
   private RoleNameCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<RoleName> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<RoleName>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            RoleNameCreateService s = (RoleNameCreateService) event.getSource();
            RoleName ent = s.getValue();
            event.consume();
            s.reset();
