...
@Singleton
public class ProcurementOrderItemEditController implements EntityController
{

   @Inject
   private ProcurementOrderItemEditView editView;

   @Inject
   private ProcurementOrderItemEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ProcurementOrderItemLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ProcurementOrderItem> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ProcurementOrderItem> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<ProcurementOrderItem>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setProcurementOrderItem(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ...
            ProcurementOrderItem entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ProcurementOrderItemLoadService s = (ProcurementOrderItemLoadService) event.getSource();
            ProcurementOrderItem entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
