...
@Singleton
public class CustomerInvoiceItemCreateController implements EntityController
{

   @Inject
   private CustomerInvoiceItemCreateView createView;

   @Inject
   private CustomerInvoiceItemCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<CustomerInvoiceItem> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getResetButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         {
            PropertyReader.copy(new CustomerInvoiceItem(), model);
         }
      });
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<CustomerInvoiceItem>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerInvoiceItemCreateService s = (CustomerInvoiceItemCreateService) event.getSource();
            CustomerInvoiceItem ent = s.getValue();
            event.consume();
            s.reset();
