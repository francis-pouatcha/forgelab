...
@Singleton
public class DocumentArchiveEditController implements EntityController
{

   @Inject
   private DocumentArchiveEditView editView;

   @Inject
   private DocumentArchiveEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private DocumentArchiveLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<DocumentArchive> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<DocumentArchive> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<DocumentArchive>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setDocumentArchive(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DocumentArchiveEditService s = (DocumentArchiveEditService) event.getSource();
            DocumentArchive entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            DocumentArchiveLoadService s = (DocumentArchiveLoadService) event.getSource();
            DocumentArchive entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent DocumentArchive p)
   {
