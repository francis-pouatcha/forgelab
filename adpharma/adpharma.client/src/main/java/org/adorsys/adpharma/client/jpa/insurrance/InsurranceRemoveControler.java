...
public class InsurranceRemoveControler
{

   @Inject
   @EntityRemoveDoneEvent
   private Event<Insurrance> removedEvent;

   @Inject
   private InsurranceRemoveService removeService;
   @Inject
   ...
   public void handleRemoveRequest(
         @Observes @EntityRemoveRequestEvent Insurrance entity)
   {
      this.entity = entity;
   ...
   public void handleRemoveOkEvent(
         @Observes @EntityRemoveConfirmedEvent Insurrance entity)
   {
   ...
   public void postConstruct()
   {
      confirmDialog.setText(resourceBundle.getString("Entity_confirm_remove.title")
            + " "
            + resourceBundle.getString("Insurrance_description.title"));
      ...
      removeService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            InsurranceRemoveService service = (InsurranceRemoveService) wse
                  .getSource();
            Insurrance p = service.getValue();
            wse.consume();
            service.reset();
            removedEvent.fire(p);
         }
