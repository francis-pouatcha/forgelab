...
public class DeliveryService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/deliverys").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public Delivery deleteById(Long id) {// @PathParam("id")
		// TODO encode id
	...
	public Delivery saveAndClose(Delivery entity) {
		Entity<Delivery> ent = Entity.entity(entity, media);
		return target().path("saveAndClose").request(media)
				.put(ent, Delivery.class);
	}
	...
	public Delivery findById(Long id) {// @PathParam("id")
	...
	public DeliverySearchResult findBy(DeliverySearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, DeliverySearchResult.class);
	}
