...
public class CompanyService
{
   ...
   public WebTarget target(){
	   return client.target(serverAddress + "/rest/companys").register(clientCookieFilter);
   }
   ...
   public Company update(Company entity)
   {
      ...
      return target().path("" + entity.getId())
            .request(media).put(ent, Company.class);
   }
   ...
   public CompanySearchResult findBy(CompanySearchInput searchInput)
   {
      ...
      return target().path(FIND_BY).request(media).post(
            searchInputEntity, CompanySearchResult.class);
   }
