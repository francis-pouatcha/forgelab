...
@Singleton
public class SectionCreateController implements EntityController
{

   @Inject
   private SectionCreateView createView;

   @Inject
   private SectionCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<Section> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Section>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SectionCreateService s = (SectionCreateService) event.getSource();
            Section ent = s.getValue();
            event.consume();
            s.reset();
