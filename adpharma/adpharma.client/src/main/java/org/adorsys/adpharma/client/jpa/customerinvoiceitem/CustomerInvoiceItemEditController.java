...
@Singleton
public class CustomerInvoiceItemEditController implements EntityController
{

   @Inject
   private CustomerInvoiceItemEditView editView;

   @Inject
   private CustomerInvoiceItemEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private CustomerInvoiceItemLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<CustomerInvoiceItem> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<CustomerInvoiceItem> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<CustomerInvoiceItem>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setCustomerInvoiceItem(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerInvoiceItemEditService s = (CustomerInvoiceItemEditService) event.getSource();
            CustomerInvoiceItem entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            CustomerInvoiceItemLoadService s = (CustomerInvoiceItemLoadService) event.getSource();
            CustomerInvoiceItem entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
