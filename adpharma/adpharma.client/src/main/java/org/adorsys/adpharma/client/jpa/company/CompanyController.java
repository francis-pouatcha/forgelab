...
@Singleton
public class CompanyController extends DomainComponentController
{

   @Inject
   private CompanySearchController searchController;

   @Inject
   private CompanyCreateController createController;

   @Inject
   private CompanyEditController editController;

   @Inject
   private CompanyListController listController;

   @Inject
   private CompanyDisplayController displayController;

   @Inject
   private CompanyIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<Company> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<Company> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<Company> selectedModelEvent;

   private Company createModel = new Company();
   private Company searchModel = new Company();
   private Company selectedModel = new Company();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<Company> entities)
   {
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent Company selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent Company selectedEntity)
   {
