...
@Singleton
public class ProcurementOrderCreateController implements EntityController
{

   @Inject
   private ProcurementOrderCreateView createView;

   @Inject
   private ProcurementOrderCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<ProcurementOrder> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<ProcurementOrder>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProcurementOrderCreateService s = (ProcurementOrderCreateService) event.getSource();
            ProcurementOrder ent = s.getValue();
            event.consume();
            s.reset();
