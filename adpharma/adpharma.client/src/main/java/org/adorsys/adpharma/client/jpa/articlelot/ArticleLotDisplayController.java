...
@Singleton
public class ArticleLotDisplayController implements EntityController
{
   ...
   private Event<ArticleLot> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<ArticleLot> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<ArticleLot>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<ArticleLot>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<ArticleLot>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private ArticleLot displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<ArticleLot> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<ArticleLot> eventData)
   {
      ...
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new ArticleLot());
   }
