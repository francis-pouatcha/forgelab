...
@Singleton
public class ProcurementOrderController extends DomainComponentController
{

   @Inject
   private ProcurementOrderSearchController searchController;

   @Inject
   private ProcurementOrderCreateController createController;

   @Inject
   private ProcurementOrderEditController editController;

   @Inject
   private ProcurementOrderListController listController;

   @Inject
   private ProcurementOrderDisplayController displayController;

   @Inject
   private ProcurementOrderIntialScreenController intialScreenController;

   @Inject
   @CreateModelEvent
   private Event<ProcurementOrder> createModelEvent;
   @Inject
   @SearchModelEvent
   private Event<ProcurementOrder> searchModelEvent;
   @Inject
   @SelectedModelEvent
   private Event<ProcurementOrder> selectedModelEvent;

   private ProcurementOrder createModel = new ProcurementOrder();
   private ProcurementOrder searchModel = new ProcurementOrder();
   private ProcurementOrder selectedModel = new ProcurementOrder();
    ...
    * @param entities
    */
   public void handleSearchResult(@Observes @EntitySearchDoneEvent List<ProcurementOrder> entities)
   {
   ...
   public void handleSelectionEvent(@Observes @EntitySelectionEvent ProcurementOrder selectedEntity)
   {
      if (!registration.canRead())
         return;
      // if result is empty: display no result.
      // else display list of procurementOrders.
    ...
    */
   public void handleSearchRequestedEvent(@Observes @EntitySearchRequestedEvent ProcurementOrder selectedEntity)
   {
      if (!registration.canRead())
         return;
      List<EntityController> displayedViews = getDisplayedViews();
   ...
   /**
    * Display search form.
    * @param selectedProcurementOrder
    */
   public void handleCreateRequestedEvent(@Observes @EntityCreateRequestedEvent ProcurementOrder templateEntity)
   {
   ...
   public void handleEditRequestedEvent(@Observes @EntityEditRequestedEvent ProcurementOrder selectedEntity)
   {
