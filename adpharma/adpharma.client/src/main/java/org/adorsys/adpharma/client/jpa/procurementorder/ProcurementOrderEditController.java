...
@Singleton
public class ProcurementOrderEditController implements EntityController
{

   @Inject
   private ProcurementOrderEditView editView;

   @Inject
   private ProcurementOrderEditService editService;
   @Inject
   private ServiceCallFailedEventHandler editServiceCallFailedEventHandler;

   @Inject
   private ProcurementOrderLoadService loadService;
   @Inject
   private ServiceCallFailedEventHandler loadServiceCallFailedEventHandler;

   @Inject
   @EntityEditCanceledEvent
   private Event<ProcurementOrder> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<ProcurementOrder> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<ProcurementOrder>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setProcurementOrder(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            ProcurementOrderEditService s = (ProcurementOrderEditService) event.getSource();
            ProcurementOrder entity = s.getValue();
            event.consume();
            s.reset();
      ...
      loadService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         public void handle(WorkerStateEvent event)
         {
            ProcurementOrderLoadService s = (ProcurementOrderLoadService) event.getSource();
            ProcurementOrder entity = s.getValue();
            event.consume();
            s.reset();
            PropertyReader.copy(entity, displayedEntity);
   ...
   public void handleEditRequestEvent(
         @Observes @EntityEditRequestedEvent ProcurementOrder p)
   {
