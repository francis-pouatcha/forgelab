...
public class DocumentStoreService {
	...
	private WebTarget target() {
		return client.target(serverAddress + "/rest/documentstores").register(
				clientCookieFilter);
	}
	...
	// @DELETE
	// @Path("/{id:[0-9][0-9]*}")
	public DocumentStore deleteById(Long id) {// @PathParam("id")
												// TODO encode id
		return target().path("" + id).request(media)
				.delete(DocumentStore.class);
	}
	...
	public DocumentStore update(DocumentStore entity) {
		...
		return target().path("" + entity.getId()).request(media)
				.put(ent, DocumentStore.class);
	}

	// @GET
	// @Path("/{id:[0-9][0-9]*}")
	// @Produces("application/xml")
	public DocumentStore findById(Long id) {// @PathParam("id")
	...
	public DocumentStoreSearchResult findBy(DocumentStoreSearchInput searchInput) {
		...
		return target().path(FIND_BY).request(media)
				.post(searchInputEntity, DocumentStoreSearchResult.class);
	}
	...
	// @Produces("application/xml")
	// @Consumes("application/xml")
	public DocumentStoreSearchResult findByLike(
