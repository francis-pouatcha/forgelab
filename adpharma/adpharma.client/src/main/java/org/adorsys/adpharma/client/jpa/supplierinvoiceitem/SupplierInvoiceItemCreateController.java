...
@Singleton
public class SupplierInvoiceItemCreateController implements EntityController
{

   @Inject
   private SupplierInvoiceItemCreateView createView;

   @Inject
   private SupplierInvoiceItemCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<SupplierInvoiceItem> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<SupplierInvoiceItem>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierInvoiceItemCreateService s = (SupplierInvoiceItemCreateService) event.getSource();
            SupplierInvoiceItem ent = s.getValue();
            event.consume();
            s.reset();
