...
@Singleton
public class EmployerDisplayController implements EntityController
{
   ...
   private Event<Employer> editRequestEvent;

   @Inject
   @EntityRemoveRequestEvent
   private Event<Employer> removeRequest;

   @Inject
   @AssocSelectionResponseEvent
   private Event<AssocSelectionEventData<Employer>> selectionResponseEvent;

   private ObjectProperty<AssocSelectionEventData<Employer>> pendingSelectionRequestProperty = new SimpleObjectProperty<AssocSelectionEventData<Employer>>();

   @Inject
   @ComponentSelectionRequestEvent
   private Event<ComponentSelectionRequestData> componentSelectionRequestEvent;

   private Employer displayedEntity;
   ...
   public void postConstruct()
   {
      ...
      displayView.getConfirmSelectionButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            final AssocSelectionEventData<Employer> pendingSelectionRequest = pendingSelectionRequestProperty.get();
   ...
   public void handleAssocSelectionRequest(@Observes(notifyObserver = Reception.ALWAYS) @AssocSelectionRequestEvent AssocSelectionEventData<Employer> eventData)
   {
      pendingSelectionRequestProperty.set(eventData);
      componentSelectionRequestEvent.fire(new ComponentSelectionRequestData(Employer.class.getName()));
      searchRequestedEvent.fire(eventData.getTargetEntity() != null ? eventData.getTargetEntity() : new Employer());
   }
   ...
   public void handleNewModelEvent(@Observes @SelectedModelEvent Employer model)
   {
	...
	public void reset() {
	     PropertyReader.copy(new Employer(), displayedEntity);
	}
}