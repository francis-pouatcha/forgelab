...
@Singleton
public class SupplierCreateController implements EntityController
{
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<Supplier>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            SupplierCreateService s = (SupplierCreateService) event.getSource();
            Supplier ent = s.getValue();
            event.consume();
            s.reset();
