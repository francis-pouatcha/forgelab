...
@Singleton
public class DocumentStoreCreateController implements EntityController
{

   @Inject
   private DocumentStoreCreateView createView;

   @Inject
   private DocumentStoreCreateService createService;
   @Inject
   private ServiceCallFailedEventHandler createServiceCallFailedEventHandler;

   @Inject
   @EntityCreateDoneEvent
   private Event<DocumentStore> createDoneEvent;
   ...
   public void init()
   {
      ...
      createView.getSaveButton().setOnAction(new EventHandler<ActionEvent>()
      {
         ...
         public void handle(ActionEvent e)
         {
            Set<ConstraintViolation<DocumentStore>> violations = createView.getView().validate(model);
      ...
      createService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            DocumentStoreCreateService s = (DocumentStoreCreateService) event.getSource();
            DocumentStore ent = s.getValue();
            event.consume();
            s.reset();
