...
@Singleton
public class CustomerInvoiceEditController implements EntityController
{

   @Inject
   private CustomerInvoiceEditView editView;

   @Inject
   private CustomerInvoiceEditService editService;
   @Inject
   ...
   private Event<CustomerInvoice> editCanceledEvent;

   @Inject
   @EntityEditDoneEvent
   private Event<CustomerInvoice> editedDoneEvent;
   ...
   public void postConstruct()
   {
      ...
      editView.getSaveButton().setOnAction(
            new EventHandler<ActionEvent>()
            {
               ...
               public void handle(ActionEvent e)
               {
                  ...
                  Set<ConstraintViolation<CustomerInvoice>> violations = editView.getView().validate(displayedEntity);
                  if (violations.isEmpty())
                  {
                     editService.setCustomerInvoice(displayedEntity).start();
                  }
                  else
                  {
      ...
      editService.setOnSucceeded(new EventHandler<WorkerStateEvent>()
      {
         ...
         {
            CustomerInvoiceEditService s = (CustomerInvoiceEditService) event.getSource();
