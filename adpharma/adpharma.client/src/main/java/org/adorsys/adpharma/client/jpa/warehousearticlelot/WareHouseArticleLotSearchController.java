...
@Singleton
public class WareHouseArticleLotSearchController implements EntityController {

	@Inject
	private WareHouseArticleLotSearchView searchView;
	...
	public void postConstruct() {
		...
		searchView.getSearchButton().setOnAction(
		...
		searchService.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
			...
			public void handle(WorkerStateEvent event) {
				WareHouseArticleLotSearchService s = (WareHouseArticleLotSearchService) event
						.getSource();
				WareHouseArticleLotSearchResult searchResult = s.getValue();
				event.consume();
				s.reset();
		...
		searchView.getResetButton().setOnAction(
				new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent e) {
						PropertyReader.copy(new DocumentStore(), model);
					}
				});
		...
		searchView.getCancelButton().setOnAction(
				new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent e) {
	 ...
	 */
	public void handleSearchRequestedEvent(
			@Observes @EntitySearchRequestedEvent DocumentStore templateEntity) {
		PropertyReader.copy(templateEntity, model);
		model.setId(null);
		model.setVersion(0);
	}

	private WareHouseArticleLotSearchInput buildSearchInputs() {
		WareHouseArticleLotSearchInput result = new WareHouseArticleLotSearchInput();
		result.setEntity(model);
		List<String> readSearchFields = PropertyReader.readSearchFields(
				searchView, model);
	...
	public void handleEntityListPageIndexChangedEvent(
			@Observes @EntityListPageIndexChangedEvent WareHouseArticleLotSearchResult searchResult) {
		searchService.setSearchInputs(searchResult.getSearchInput()).start();
	}
	
	public void reset() {
	     PropertyReader.copy(new WareHouseArticleLot(), model);
	}
